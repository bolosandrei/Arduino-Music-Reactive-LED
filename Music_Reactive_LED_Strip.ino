// PMP Arduino Music Reactive LED Strip Project
#include "FastLED.h"
#include <FastLED.h>
...
//int noise[] = {204,188,68,73,150,98,88,68}; // noise level determined by playing pink noise and seeing levels
//int noise[] = {204,190,108,85,65,65,55,60}; // noise for mega adk
//int noise[] = {204,195,100,90,85,80,75,75}; // noise for NANO
float noise[] = {204,198,100,85,85,80,80,80};
float noise_fact[]     = {15, 7, 1.5, 1, 1.2, 1.4, 1.7,3}; // BASE noise level determined by playing pink noise and seeing levels
float noise_fact_adj[] = {15, 7, 1.5, 1, 1.2, 1.4, 1.7,3}; // Amplified with master_volume

float eq[] = {150, 160, 150, 120, 90, 90, 95, 100}; //equalizer - boost <- normal * EQ value[i]
...
//#define NUM_LEDS    297
int counter2=0;
CRGB leds[NUM_LEDS]; //the led strip
...
void setup() { 
  Serial.begin(115200); // to see the values we get from analogRead, we open a terminal
  delay(1000);
...
bool eq_on = true;
 ...
 while (1) { // reduces jitter
      ...
      for (int i = 0 ; i < FHT_N ; i++) { // save 256 samples
          ...
          byte j = ADCH; // concatenate the lower byte to the higher byte
          int k = (j << 8) | m; // form into an int
          k -= 0x0200; // form into a signed int
          k <<= 6; // form into a 16b signed int
          fht_input[i] = k; // put real-recorded data into bins
        }
  ...
  //  Serial.println (master_volume);
        // amplify with volume/intensity from potentiometer for each octave(freq)
        for (int i=1; i<8; i++) {
      ...
      for (int i = 2; i < 8; i++) {  // goes through each octave. skip the first 1, which is not useful
        ...
        if (j<10) {j=0;}
          ...
          j = j * noise_fact_adj[i];}
           ...
           Serial.print(i);
           Serial.print("| "); 
          if (j>=140) { // for higher volume intensity
  //         Serial.print("i: "); 
  //         Serial.print(i);
  //         Serial.print(", "); 
             if(j>=200) Serial.print(" ---o<#@@@@@@@@#>o--- "); 
            if (i>=7) {beat+=2;} // higher octave
             else
            if ((i==2) && j>=200) {beat+=7;} // lower octave and high volume intensity
             else
            if(i==2 || i==3) {beat+=2;} // lower octave and low volume intensity
            else {beat+=1;}
          }
          j=j/30;
  ...
  // this fills in 11 LED's with interpolated values between each of the 8 OCT values 
         if (i>=2) {
          ...
          for (uint8_t y=0;y<kMatrixHeight;y++){ // sets each led's color using the mapping function 
        ...
        if (beat>=6) { // when a high beat is encounered
  //          fill_solid(leds, NUM_LEDS, CRGB::Grey);
  //          FastLED.setBrightness(150); 
  //          fill_solid(leds, NUM_LEDS, CRGB::White);
  //          FastLED.setBrightness(200); 
//            fill_solid(leds, NUM_LEDS, CRGB(random(25,255),random(25,255),random(25,255))); // set all leds to a random color
...
uint16_t XY( uint8_t x, uint8_t y)// mapping function
{
  uint16_t p;
  ...
  if( kMatrixSerpentineLayout == true) {
    if( y & 0x01) {
      ...
      p = (y * kMatrixWidth) + reverseX;
    } else {
      // Even rows run forwards
      p = (y * kMatrixWidth) + x;
    }
  }
  p = (p+counter2) % NUM_LEDS;  
  return p;
}
